...
function DashCard({ onRefresh, onHighlight }) {
  ...
  const fetchVesselData = async (imoNumber) => {
    try {
      const response = await axios.get("http://104.225.218.43:5000/api/ais-data", {
  ...
  useEffect(() => {
    const fetchVessels = async () => {
      try {
        const response = await axios.get("http://104.225.218.43:5000/api/get-vessels", {
  ...
  const handleAddToTrack = async () => {
    ...
    try {
      ...
      if (result.isConfirmed) {
        ...
        const aisResponse = await axios.get("http://104.225.218.43:5000/api/ais-data", {
          params: { imo: imoNumber }
        });
        ...
        await axios.post('http://104.225.218.43:5000/api/add-combined-data', requestBody);
